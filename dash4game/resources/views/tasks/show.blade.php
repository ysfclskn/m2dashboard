<x-app-layout :current-project="$project">
    ...
    <div class="max-w-3xl space-y-4">
        <div class="bg-gray-900 border border-gray-800 rounded-xl p-6">
            ...
            <div class="grid grid-cols-2 gap-4 text-sm mb-4">
                ...
                <div>
                    <span class="text-gray-500">Linked Wiki:</span>
                    @if ($task->wikiPage)
                        <a href="{{ route('projects.wiki.show', [$project, $task->wikiPage]) }}" class="text-amber-400 hover:text-amber-300 ml-2 inline-flex items-center gap-1 font-medium hover:underline">
                            📖 {{ $task->wikiPage->title }}
                        </a>
                    @else
            ...
            @if ($task->description)
                <div class="border-t border-gray-800 pt-4">
                    <div id="task-description" class="wiki-prose text-sm">
                        <div id="task-description-fallback" class="whitespace-pre-wrap text-gray-300 leading-relaxed">{{ $task->description }}</div>
                    </div>
                </div>
            @endif
    ...
    @if ($task->description)
        @push('scripts')
        <script>window._taskRaw = @json($task->description);</script>
        <script src="https://cdn.jsdelivr.net/npm/marked@9/marked.min.js"></script>
        <script src="https://cdn.jsdelivr.net/npm/dompurify@3/dist/purify.min.js"></script>
        <script>
        document.addEventListener('DOMContentLoaded', function () {
            const container = document.getElementById('task-description');
            const fallback  = document.getElementById('task-description-fallback');
            if (!container || typeof marked === 'undefined') return;
            const html = typeof DOMPurify !== 'undefined'
                ? DOMPurify.sanitize(marked.parse(window._taskRaw))
                : marked.parse(window._taskRaw);
