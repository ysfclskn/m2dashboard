<x-app-layout :current-project="$project">
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                @if ($activeSprint)
                    <span class="text-xs px-2 py-0.5 rounded-full text-green-400 bg-green-900/30">⚡ {{ $activeSprint->name }}</span>
                @endif
            </div>
    ...
    @if (!$activeSprint)
        {{-- No active Raid Sprint empty state --}}
        <div class="bg-gray-900 border border-gray-800 rounded-xl p-12 text-center">
            <p class="text-4xl mb-4">⚡</p>
            <h2 class="text-lg font-semibold text-gray-100 mb-2">No Active Raid Sprint</h2>
            <p class="text-gray-500 text-sm mb-6">Start or activate a Raid Sprint to open the Quest Board.</p>
            @can('update', $project)
